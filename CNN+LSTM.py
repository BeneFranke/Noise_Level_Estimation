...
class Conv_LSTM(nn.Module):                      # torch.Size([batch_size, n_channels=1, pxl_x, pxl_y])
    def __init__(self, hidden_size, num_layers):
        ...
        # lstm network
        self.lstm = nn.LSTM(input_size=32 * 14 * 14, num_layers=self.num_layers, hidden_size=self.hidden_size)
    ...
    def forward(self, x):
        ...
        x = x.view(-1, 32 * 14 * 14)
        x, hidden = self.lstm(x.view(x.shape[0], 1, 32 * 14 * 14))
...
def train(train_loader, model, optimizer, criterion, n_epochs, batch_size):
    ...
    for epoch in range(n_epochs):
        ...
        for i, (x, y) in enumerate(train_loader):
            # reshape input to 4D
            x = np.reshape(x, [batch_size, 1, x.shape[1], x.shape[2]])           # [455, 128] -> [455, 128, 1, 1]
            y = np.reshape(y, [batch_size, 1])
            # print(x.shape)
            # [batch_size, N_channels=1, Height, Width]

            # workaround break to prevent error
            # if i == len(train_loader)-1:          <--- FIXED IT! Not necessary anymore!
            #    break
...
if __name__ == "__main__":
    ...
    # initialize model
    model = Conv_LSTM(hidden_size=N_HIDDEN, num_layers=N_LAYERS)
    ...
    plt.yscale("log")
    plt.title('Cost function of Hybrid model')
    plt.xlabel('Epoch')
    plt.ylabel('MSE [dB]')
    plt.grid(True)
    plt.savefig(path + "\\Plots\\error_func\\ef_hybrid_log")
    plt.close()
    ...
    plt.ylabel('MSE [dB]')
    plt.grid(True)
    plt.savefig(path + "\\Plots\\error_func\\ef_hybrid")
    plt.show()

    # testing loop
    test_loss, test_rmse, test_rmse_hist = test(test_loader=test_loader, model=model, criterion=criterion, batch_size=batch_size_test)
    ...
    # save detailed testing results for any sample  --> THIS ONLY WORKS WITH BATCH_SIZE = 1
    df = pd.DataFrame(data=test_results, columns=["y_true", "y_pred", "y_dif", "class", "recording"])
    df.to_csv(path+"\\Results\\Hybrid_Model_"+str(N_MEL)+"features_"+str(N_EPOCHS)+"epochs.csv",
              index=False, sep=";")
    ...
    df.to_csv(path+"\\Results\\RMSE\\Hybrid_Model_"+str(N_MEL)+"features_"+str(N_EPOCHS)+"epochs.csv", sep=";")

    # save model
    torch.save(model, open(path + "\\Models\\Hybrid.sav", 'wb'))

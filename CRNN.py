...
class Basic_CRNN(nn.Module):
    def __init__(self, hidden_size, num_layers):
        ...
        # recurrent layer
        self.rnn = nn.RNN(input_size=32 * 14 * 14, hidden_size=self.hidden_size, num_layers=self.num_layers)

        # linear layers
        self.fc1 = nn.Linear(self.hidden_size, 120)                # Fc_in = C * H_out * W_out
    ...
    def init_hidden(self):
        # initialize first hidden state h0
        # the dimension axes are (num_layers, batch_size, hidden_size)
        return torch.zeros(self.num_layers, self.batch_size, self.hidden_size)

    def forward(self, x):
        # Conv layer with ReLU, BatchNorm and 2x2 pooling
        ...
        # recurrent layers
        x, hidden = self.rnn(x.view(x.shape[0], 1, 32 * 14 * 14))
        # print(x.shape)
        # x = self.pool1d(x)
...
if __name__ == "__main__":
    ...
    # initialize model
    model = Basic_CRNN(hidden_size=N_HIDDEN, num_layers=N_LAYERS)
    ...
    optimizer = torch.optim.Adam(model.parameters(), lr=LEARNING_RATE)          # nn.torch.optim.SSG

    # create Datasets and Dataloader
    train_data = AudioDataset(X=X_train, Y=y_train)#, label=label_train)
    test_data = AudioDataset(X=X_test, Y=y_test)#, label=label_test)
    ...
    plt.yscale("log")
    plt.title('Cost function of CRNN model')
    plt.xlabel('Epoch')
    plt.ylabel('MSE [dB]')
    plt.grid(True)
    plt.savefig(path + "\\Plots\\error_func\\ef_crnn_log_300")
    plt.close()
    ...
    plt.ylabel('MSE [dB]')
    plt.grid(True)
    plt.savefig(path + "\\Plots\\error_func\\ef_crnn_300")
    plt.show()
    ...
    df.to_csv(path+"\\Results\\CRNN_BN_"+str(N_MEL)+"features_"+str(N_EPOCHS)+"epochs.csv", index=False, sep=";")
    ...
    df.to_csv(path+"\\Results\\RMSE\\CRNN_BN_"+str(N_MEL)+"features_"+str(N_EPOCHS)+"epochs.csv", sep=";")

    # save model
    torch.save(model, open(path + "\\Models\\CRNN.sav", 'wb'))
